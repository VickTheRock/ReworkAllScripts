...
namespace ControlCreep_By_Vick
{
    internal class Program
    {
        ...
        public static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (activated && (target.IsAlive && !target.IsInvul() && Game.MousePosition.Distance2D(target) <= 1000 || target.Distance2D(me) <= 600))
            {
                ...
                var Neutral = ObjectMgr.GetEntities<Creep>().Where(creep => (creep.ClassID == ClassID.CDOTA_BaseNPC_Creep_Neutral &&
                 creep.IsAlive && creep.IsControllable)).ToList();


                if (target.Distance2D(me) <= 1200)
                {
                    foreach (var v in Neutral)
                    {

                        if (v.Name == "npc_dota_neutral_dark_troll_warlord")
                        {
                        ...
                        if (v.Name == "npc_dota_neutral_big_thunder_lizard")
                        ...
                        if (v.Name == "npc_dota_neutral_centaur_khan")
                        {
                        ...
                        if (v.Name == "npc_dota_neutral_satyr_hellcaller")
                        {
                        ...
                        if (v.Name == "npc_dota_neutral_polar_furbolg_ursa_warrior")
                        {
                    ...
                    foreach (var v in spiritbear)
                        if ((!me.AghanimState() && me.Position.Distance2D(v) <= 1200) || me.AghanimState())
                        {
                            ...
                            var mom = v.FindItem("item_mask_of_madness");
                            ...
                            if (target.Position.Distance2D(v.Position) < 600 && mom.CanBeCasted() &&
                                Utils.SleepCheck(v.Handle.ToString()))
                            {
                    ...
                    foreach (var v in Wolf)
                    {
                        ...
                        //This is golem time.
                        if (target.Position.Distance2D(v.Position) < 600 && v.Spellbook.Spell1.CanBeCasted() && (!CheckSetka || !CheckStun || !target.IsHexed() || !target.IsStunned()) &&
                            Utils.SleepCheck(v.Handle.ToString()))
                        {
                            v.Spellbook.Spell1.UseAbility(target);
                            Utils.Sleep(400, v.Handle.ToString());
                        }
        ...
        static void Drawing_OnEndScene(EventArgs args)
        {
            ...
            if (!activated)
            {
                txt.DrawText(null, "Creep Control Off BackSpace", 1200, 27, Color.MediumOrchid);
            }
        }
